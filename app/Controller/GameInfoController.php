...
class GameInfoController extends AppController {
	...
	public function index() {
		...
		// 模糊搜索的条件
		$keyword = $this->request->query('寻找游戏');
		...
		} else if (!empty($ass1) || !empty($ass2) || !empty($ass3) || !empty($ass4) || !empty($ass5)) {
			// 精确查询

			// 初始化条件
			$conditions = [];
			if ($ass1) {
				$conditions[] = array('game_name LIKE' => "%" . $ass1 . '%');
			}
			if ($ass2) {
				$conditions[] = array('type LIKE' => "%" . $ass2 . '%');
			}
			if ($ass3) {
				$conditions[] = array('score LIKE' => "%" . $ass3 . '%');
			}
			$this->GameInfo->clear();

			$gameInfoallList = $this->GameInfo->find("all", array(
				'limit' => $pagenation['itemPerPage'],
				'page' => $page,
	...
	public function delete($id) {
		...
		if ($this->GameInfo->delete($id)) {
			$this->Flash->success(__('你删除了一个编号为: %s 号的数据', h($id)));
		} else {
			$this->Flash->error(__('你想删除编号为: %s 号的数据却失败了.', h($id)));
		}
